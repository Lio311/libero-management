                    ...
                    <div className="backdrop-blur-2xl bg-zinc-950/40 border border-white/10 text-slate-300 rounded-3xl p-10 shadow-2xl overflow-hidden relative">
                        ...
                        <div className="relative z-10 flex flex-col items-center">
                            <motion.div 
                                ...
                                className="w-16 h-16 glass-panel/10 rounded-2xl flex items-center justify-center mb-8 border border-white/20 shadow-inner overflow-hidden"
                            >
                                {influencerImage ? (
        ...
        <div className="space-y-8 animate-in fade-in duration-500 px-6 md:px-12 pt-8 pb-20 max-w-7xl mx-auto" dir="rtl">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl md:text-3xl font-bold text-white">{influencerName || 'משפיענית'}</h1>
            ...
            {error ? (
                ...
                <div className="glass-panel rounded-3xl border border-white/10 text-slate-300 p-32 flex flex-col items-center justify-center shadow-sm">
                    ...
                    <p className="text-slate-300 text-sm mt-1 text-center">מסנכרן הזמנות מהאתרים השונים</p>
                </div>
            ) : (
                <>
                    {/* Summary Cards */}
                    {summary && (
                        <div className="space-y-8">
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
                                ...
                                <div className="glass-panel rounded-3xl p-6 border border-white/10 text-slate-300 shadow-sm">
                                    <p className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">סה"כ מכירות (כולל מע"מ)</p>
                                    <p className="text-2xl font-black text-white" dir="ltr">{formatILS(summary.total_revenue)}</p>
                                </div>
                                <div className="glass-panel rounded-3xl p-6 border border-white/10 text-slate-300 shadow-sm">
                                    <p className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">ממוצע להזמנה (כולל מע"מ)</p>
                                ...
                                {influencerId !== 'moran' && (
                                    <>
                                        <div className="glass-panel rounded-3xl p-6 border border-white/10 text-slate-300 shadow-sm">
                                            <p className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">סה"כ עמלה {hasVat ? '(כולל מע"מ)' : ''}</p>
                                            ...
                                            {commissionRateStr && (!summary.coupon_rates || Object.keys(summary.coupon_rates).length === 0) && (
                                                <p className="text-sm font-medium text-slate-300 mt-1">לפי {commissionRateStr} מהמכירות</p>
                                            )}
                                        </div>
                                        {!!summary.base_salary && (
                                            <div className="glass-panel rounded-3xl p-6 border border-white/10 text-slate-300 shadow-sm">
                                                <p className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">שכר בסיס</p>
                                                <p className="text-2xl font-black text-white" dir="ltr">{formatILS(summary.base_salary)}</p>
                                            </div>
                                        )}
                                        {!!summary.monthly_bonus && (
                                            <div className="glass-panel rounded-3xl p-6 border border-white/10 text-slate-300 shadow-sm">
                                                <p className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">תוספת חודשית</p>
                                                <p className="text-2xl font-black text-white" dir="ltr">{formatILS(summary.monthly_bonus)}</p>
                                            </div>
                                        )}
                                ...
                                {summary.duduar_bottles !== undefined && (
                                    <>
                                        <div className="glass-panel rounded-3xl p-6 border border-white/10 text-slate-300 shadow-sm">
                                            <p className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">בקבוקי דודואר</p>
                                            <p className="text-2xl font-black text-white">{summary.duduar_bottles}</p>
                                        </div>
                                        <div className="glass-panel rounded-3xl p-6 border border-white/10 text-slate-300 shadow-sm">
                                            <p className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">מכירות דודואר (כולל מע"מ)</p>
                                            <p className="text-2xl font-black text-white" dir="ltr">{formatILS(summary.duduar_revenue || 0)}</p>
                                        </div>
                                        {influencerId !== 'moran' && (
                            ...
                            {summary.brand_summaries && Object.keys(summary.brand_summaries).length > 1 && (
                                <div className="mt-8 space-y-4 pt-4 border-t border-black/[0.05]">
                                    <h3 className="text-lg font-bold text-white mb-2">פירוט לפי מותג</h3>
                                    {Object.entries(summary.brand_summaries).map(([brand, bs]) => (
                                        <div key={brand} className="glass-panel/70 p-5 rounded-2xl border border-white/10 text-slate-300 flex flex-col md:flex-row md:items-center gap-6">
                                            <div className="flex items-center gap-3 md:w-48 flex-shrink-0">
                                                ...
                                                <h4 className="font-bold text-white text-lg">{BRAND_LABELS[brand] || brand}</h4>
                                            </div>
                                            <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                                                <div className="glass-panel p-3 rounded-xl border border-white/10 text-slate-300/50 shadow-sm">
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">הזמנות</p>
                                                    <p className="text-lg font-black text-white">{bs.total_orders}</p>
                                                </div>
                                                <div className="glass-panel p-3 rounded-xl border border-white/10 text-slate-300/50 shadow-sm">
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">מוצרים</p>
                                                    <p className="text-lg font-black text-white">{bs.total_items}</p>
                                                </div>
                                                <div className="glass-panel p-3 rounded-xl border border-white/10 text-slate-300/50 shadow-sm">
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">מכירות (כולל מע"מ)</p>
                                                    <p className="text-lg font-black text-white" dir="ltr">{formatILS(bs.total_revenue)}</p>
                                                </div>
                                                {influencerId !== 'moran' && (
                                                    <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100/50 shadow-sm flex flex-col justify-center">
                                                        <p className="text-[10px] font-bold text-blue-300/80 uppercase tracking-wider mb-0.5">עמלה {hasVat ? '(כולל מע"מ)' : ''}</p>
                                                        <p className="text-lg font-black text-blue-300 leading-tight" dir="ltr">{formatILS(bs.commission)}</p>
                                                        {hasVat && (
                                                            <p className="text-[9px] font-medium text-blue-300/70 mt-0.5 leading-tight" dir="ltr">{formatILS(bs.commission / 1.18)} לא כולל מע"מ</p>
                                                        )}
                                                        {getBrandCommissionRateStr(brand) && (
                                                            <p className="text-[9px] font-medium text-blue-300/70 mt-0.5 leading-tight">לפי {getBrandCommissionRateStr(brand)} מהמכירות</p>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
                            ...
                            <div className="glass-panel rounded-3xl p-6 rounded-3xl border border-white/10 text-slate-300 shadow-sm mt-8">
                                <h3 className="text-xl font-bold text-white mb-4">הגדרות תשלום לחודש זה</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-200 mb-2">תוספת חודשית חד פעמית (₪)</label>
                                        <input type="number" value={monthlyBonus} onChange={e => setMonthlyBonus(Number(e.target.value))} className="w-full border border-white/20 rounded-xl p-3 glass-panel focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-200 mb-2">אחוזי עמלה לקופונים (%)</label>
                                            ...
                                            {influencerConfig?.coupons?.map(coupon => (
                                                <div key={coupon.code} className="flex items-center gap-3">
                                                    ...
                                                    <input type="number" 
                                                        ...
                                                        className="flex-1 border border-white/20 rounded-xl p-2 glass-panel focus:ring-2 focus:ring-blue-500 outline-none transition-all" 
                                                    />
                                                </div>
                                            ))}
                                            {(!influencerConfig?.coupons || influencerConfig.coupons.length === 0) && (
                                                <p className="text-sm text-slate-400">לא נמצאו קופונים מוגדרים</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
                    ...
                    {orders.length === 0 ? (
                        <div className="glass-panel rounded-3xl border border-white/10 text-slate-300 p-24 flex flex-col items-center justify-center text-center shadow-sm">
                            ...
                            <p className="text-slate-400 max-w-xs mt-2 font-medium">לא נמצאו הזמנות עם הקופונים המשויכים בחודש הנבחר.</p>
                        </div>
                    ) : (
                        <div className="glass-panel rounded-3xl border border-white/10 text-slate-300 shadow-sm overflow-hidden">
                            <div className="px-6 py-5 border-b border-white/10 text-slate-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                <div>
                                    <h2 className="text-lg font-bold text-white">
                                        פירוט הזמנות — {format(currentDate, 'MMMM yyyy', { locale: he })}
                                    </h2>
                                    <p className="text-sm text-slate-400 mt-0.5">{orders.length} הזמנות</p>
                                </div>
                            </div>
                                                ...
                                                <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            ...
                                                            <span className="px-2 py-0.5 rounded-full text-[10px] md:text-xs font-bold bg-slate-100 text-slate-200 font-mono">
                                                                {order.coupon_used}
                                                            </span>
                                                        </div>
                                                ...
                                                <div className="px-3 md:px-6 pb-5 bg-transparent">
                                                    <div className="glass-panel rounded-2xl border border-white/10 text-slate-300 overflow-hidden shadow-sm">
                                                        ...
                                                        <div className="px-4 md:px-5 py-3 md:py-4 glass-panel border-t border-white/10 text-slate-300 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                                                            ...
                                                            <div className="font-bold text-base md:text-lg text-blue-300 w-full sm:w-auto flex justify-between sm:justify-start border-t border-white/20 sm:border-0 pt-2 sm:pt-0 mt-1 sm:mt-0" dir="rtl">
                                                                <span>סה"כ:</span>
