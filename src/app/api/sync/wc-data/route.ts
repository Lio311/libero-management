...
import { wcProducts, wcOrders, velourProducts, velourOrders, laburaProducts, laburaOrders, qcProducts, priceHistory } from '@/lib/db/schema';
...
export async function GET(request: Request) {
  ...
  const store = (url.searchParams.get('store') as 'libero' | 'velour' | 'labura') || 'libero';

  const targetProductsTable = store === 'velour' ? velourProducts : store === 'labura' ? laburaProducts : wcProducts;
  const targetOrdersTable = store === 'velour' ? velourOrders : store === 'labura' ? laburaOrders : wcOrders;
  let queryParams = '';
  ...
  try {
    ...
    // 2. Fetch & Sync Orders
    const orders = await fetchFromWooCommerce('orders', queryParams + '&status=processing,completed,cancelled,refunded,failed,trash,on-hold&_fields=id,total,date_created_gmt,date_modified_gmt,date_completed_gmt,line_items,shipping_lines,customer_id,status,billing,customer_note', store);
