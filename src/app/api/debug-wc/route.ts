...
export async function GET(request: Request) {
  ...
  const wcUrl = `${config.baseUrl}/wp-json/wc/v3/orders?per_page=5&consumer_key=${config.ck}&consumer_secret=${config.cs}`;
  
  try {
    const res = await fetch(wcUrl);
    const data = await res.json();
    
    // Dump just the meta_data and shipping_lines of the first order
